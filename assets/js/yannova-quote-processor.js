...
class YannovaQuoteProcessor {
  ...
  assessUrgency(klantData) {
    ...
    if (klantData.gewensteDatum) {
      const dagen = Math.ceil(
        (new Date(klantData.gewensteDatum) - new Date()) /
          (1000 * 60 * 60 * 24)
      );
  ...
  checkVentilationNeeds(requestData) {
    ...
    if (
      requestData.locatieType === 'kantoor' ||
      requestData.locatieType === 'school'
    ) {
      return 'hoog';
    }
    return 'normaal';
  ...
  checkAccessibilityNeeds(requestData) {
    ...
    if (
      requestData.locatieType === 'zorginstelling' ||
      requestData.locatieType === 'school'
    ) {
      return 'hoog';
    }
  ...
  assessSoundInsulationNeeds(requestData) {
    if (
      requestData.locatieType === 'kantoor' ||
      requestData.locatie.bouwjaar < 1980
    ) {
      return 'hoog';
    }
    return 'normaal';
  ...
  hasSpecialDimensions(requestData) {
    if (requestData.ramen) {
      return requestData.ramen.some(
        (raam) =>
          raam.afmetingen &&
          (raam.afmetingen.breedte > 200 || raam.afmetingen.hoogte > 250)
      );
    }
    return false;
  }
  ...
  async generateRecommendations(analysis) {
    ...
    if (analysis.requirements.duurzaamheid.energie.subsidie) {
      recommendations.push({
        ...
        beschrijving:
          'U komt in aanmerking voor ISDE subsidie. Wij helpen u met de aanvraag.',
  ...
  generateMaterialRecommendation(analysis) {
    ...
    if (klantType === 'zakelijk' || projectType === 'nieuwbouw') {
      return {
        ...
        beschrijving:
          'Voor zakelijke projecten is aluminium de beste keuze: duurzaam en professioneel.',
  ...
  generateMaintenanceRecommendation(analysis) {
    return {
      ...
      beschrijving:
        'Verleng de levensduur en behoud garantie met jaarlijks onderhoud.',
      kosten: '€75-150 per jaar',
      voordelen: [
        '10 jaar garantie behouden',
